...
If we look at the read-menu program from [Chapter 28](https://learning.oreilly.com/library/view/the-linux-command/9781492071235/xhtml/ch28.xhtml#ch28), we see the logic used to act on a user’s selection.
...
The case command looks at the value of word, which in our example is the value of the REPLY variable, and then attempts to match it against one of the specified _patterns_. When a match is found, the commands associated with the specified pattern are executed. After a match is found, no further matches are attempted.

#### **_Patterns_**

The patterns used by case are the same as those used by pathname expansion. Patterns are terminated with a ) character. [Table 31-1](https://learning.oreilly.com/library/view/the-linux-command/9781492071235/xhtml/ch31.xhtml#ch31tab01) describes some valid patterns.
...
|*.txt)|Matches if word ends with the characters _.txt_.|
