...
This chapter covers the ARMv7 architecture as defined in the _ARM Architecture Reference Manual_: _ARMv7-A and ARMv7-R Edition_ (ARM DDI 0406B).
...
Lines 1 and 5 simply load those globals into a register (we explain this syntax later). Line 8 iterates over the KeNodeBlock array (R0 is the base), R3 is the index multiplied by 2 (because it is an array of pointers; pointers are 4 bytes in size on this platform). Lines 10–13 initialize some fields of the KNODE element. Line 14 increments the index. Line 17 compares the index against the size of the array (R1 is the size; see line 4) and if it is less than the size then continues the loop.
...
Clearly, this is not valid syntax according to the previous section. Technically, these are called _pseudo-instructions_ and they are used by disassemblers to make manual inspection easier. Internally, they use the immediate form of LDR with PC as a base register; sometimes, this is called _PC-relative addressing_ (or _RIP-relative addressing_ on x64). ARM binaries usually have a literal pool that is a memory area in a section to store constants, strings, or offsets that others can reference in a position-independent manner. (The literal pool is part of the code, so it will be in the same section.) In the preceding snippet, the code is referencing a 32-bit constant, a string, and an offset to an imported function stored in the literal pool. This particular pseudo-instruction is useful because it allows a 32-bit constant to be moved into a register in one instruction. To make it clearer, consider the following snippet:
...
Line 5 stores a memory address in R6; the content of this memory address (0x1050c) is three words (line 17). Lines 6–8 set R0–R2 with some constants. Line 9 loads three words into R3–R5, starting at the memory location specified by R6. As shown in lines 24–26, R3–R5 contain the expected value. Line 10 stores R0–R2, starting at the memory location specified by R6. Line 29 shows that the expected values were written. [Figure 2.2](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0002) illustrates the result of the preceding operations.

[Figure 2.2](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0002)

![[../../assets/Pasted image 20260518153548.png]]
...
Line 9 uses IA mode with writeback, so the r6 is updated with an address 4 bytes above the last location (line 23). The same can be observed in lines 10, 27, and 30. [Figure 2.3](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0003) shows the result of the preceding snippet.

[Figure 2.3](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0003)

![[../../assets/Pasted image 20260518153607.png]]
...
[Figure 2.4](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0004) illustrates the preceding snippet.

[Figure 2.4](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0004)

![[../../assets/Pasted image 20260518153620.png]]
...
Unlike x86/x64, which has only one instruction for function invocation (CALL) and branching (JMP), ARM offers several depending on how the destination is encoded. When you call a function, the processor needs to know where to resume execution after the function returns; this location is typically referred to as the _return address_. In x86, the CALL instruction implicitly pushes the return address on the stack before jumping to the target function; when it is done executing, the target function resumes execution at the return address by popping it off the stack into EIP.
...
03: A4 4A A0 E1   MOV     R4, R4, LSR #21 ; r4 = (r4![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781118787397/files/images/gg.gif)21)
...
03: 04 EB 80 00   ADD.W    R0, R4, R0,LSL#2 ; r0 = r4 + (r0![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781118787397/files/images/ll.gif)2)
...
07: 83 EA C1 03   EOR.W    R3, R3, R1,LSL#3 ; r3 = r3 ^ (r1![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781118787397/files/images/ll.gif)3)
08: 53 40         EORS     R3, R2           ; r3 = r3 ^ r2 (flag)
09: 43 EA 02 23   ORR.W    R3, R3, R2,LSL#8 ; r3 = r3 | (r2![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781118787397/files/images/ll.gif)8)
...
Every example discussed so far has been executed in a linear manner. Most programs will have conditionals and loops. At the assembly level, these constructs are implemented using conditional flags, which are stored in the application program status register (APSR). The APSR is an alias of the CPSR and is similar to the EFLAG in x86. [Figure 2.5](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0005) illustrates the relevant flags, described as follows:
...
[Figure 2.5](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0005)

![[../../assets/Pasted image 20260518153653.png]]

The N, Z, C, and V bits are identical to the SF, ZF, CF, and OF bits in the EFLAG register on x86. They are used to implement conditionals and loops in higher-level languages; they are also used to support conditional execution at the instruction level. Equality is described in terms of these flags. [Table 2.1](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-tbl-0001) shows common relationships and corresponding flags.

[**Table 2.1**](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-tbl-anc-0001) Conditional code and meaning
...
When an ARM core boots up, it starts executing code in the ARM state at the memory address 0x00000000 or 0xFFFF0000, depending on a setting in coprocessor 15. This is determined by the vector (V) bit in the system control register (CP15, C1/C0). If it is 0, then the exception vector is at 0x00000000; otherwise, it is at 0xFFFF0000. This address is usually in flash memory (RAM has not been initialized yet so it cannot be used), and the content therein is commonly known as the _exception vectors_. ARM has a list of predefined vectors starting at the base address. The RESET exception handler is first in the table so it is executed after a reset event. Because it is the first code to be executed, it usually begins by performing basic hardware configuration and starts the boot process. Here is an exception vector taken from a real device:
...
Having learned all the fundamentals, you can apply them in this section by fully decompiling an unknown function. This function encompasses many concepts and techniques covered in this chapter, so it is an excellent way to put your knowledge to the test. Along the way, you will also learn new skills that were only hinted at in the early sections. Because the function is somewhat long, we put it in graph form to save space and improve readability. The function body is shown in [Figure 2.6](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0006), and all the code line numbers discussed in this section refer to this figure.

[Figure 2.6](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0006)

![[../../assets/Pasted image 20260518153724.png]]
...
Line 28 loads another field from struct1 into R3; line 29 loads from offset zero of the same structure into R0; and line 30 sets R2 to R3*3 (=R3+(R3![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781118787397/files/images/ll.gif)1)). Line 31 loads a field from struct2 into R3 and then accesses another field using that as a base pointer. This implies that you have a pointer to another structure inside struct2 at offset 0xC. Line 32 loads a field from that new structure into R3; line 33 updates it to be R3+R2*8; and line 34 uses that as a base address and loads a signed short value at offset 0x16 of another structure into R4.
...
[Figure 2.7](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0007) illustrates the relationships between the four structures.

[Figure 2.7](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0007)

![[../../assets/Pasted image 20260518153800.png]]

Here is the pseudo-code for lines 28–35:

r3 = arg1->field0c_i;
r2 = r3 + r3![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781118787397/files/images/ll.gif)1
   = arg1->field0c_i*3;
r3 = arg2->field0c_p;
...
**1.** [Figure 2.8](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0008) shows a function that takes two arguments. It may seem somewhat challenging at first, but its functionality is very common. Have patience.

[Figure 2.8](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0008)

![[../../assets/Pasted image 20260518153815.png]]

**2.** [Figure 2.9](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0009) shows a function that was found in the export table.

[Figure 2.9](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0009)

![[../../assets/Pasted image 20260518153828.png]]
...
**4.** [Figure 2.10](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0010) shows another easy function.

[Figure 2.10](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0010)

![[../../assets/Pasted image 20260518153835.png]]

**5.** [Figure 2.11](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0011) is simple as well. The actual string names have been removed so you cannot cheat by searching the Internet.

[Figure 2.11](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0011)

	![[../../Pasted image 20260518153841.png]]

**6.** [Figure 2.12](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0012) involves some twiddling.

[Figure 2.12](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0012)

![[../../assets/Pasted image 20260518153850.png]]

**7.** [Figure 2.13](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0013) illustrates a common routine, but you may not have seen it implemented this way.

[Figure 2.13](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0013)

![[../../assets/Pasted image 20260518153857.png]]

**8.** In [Figure 2.14](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0014), byteArray is a 256-character array whose content is byteArray[] = {0, 1, …, 0xff}.

[Figure 2.14](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0014)

![[../../assets/Pasted image 20260518153905.png]]

**9.** What does the function shown in [Figure 2.15](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0015) do?

[Figure 2.15](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0015)

![[../../assets/Pasted image 20260518153912.png]]

**10.** [Figure 2.16](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0016) is a function from Windows RT. Read MSDN if needed. Ignore the security PUSH/POP cookie routines.

[Figure 2.16](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0016)

![[../../assets/Pasted image 20260518153918.png]]

**11.** In [Figure 2.17](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-0017), sub_101651C takes three arguments and returns nothing. If you complete this exercise, you should pat yourself on the back.

[Figure 2.17](https://learning.oreilly.com/library/view/practical-reverse-engineering/9781118787397/9781118787397c02.xhtml#c02-fig-anc-0017)

![[../../assets/Pasted image 20260518153926.png]]